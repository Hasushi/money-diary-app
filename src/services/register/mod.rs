...
fn input_register_type() -> u8 {
    ...
    let register_type = register_type.trim().parse().expect("数値を入力してください");
...
fn input_category_type(register_type: u8) -> u8 {
    ...
    io::stdin().read_line(&mut category_type).expect("カテゴリーの入力に失敗しました");
    let category_type: u8 = category_type.trim().parse().expect("数値を入力してください");
...
fn input_price() -> u32 {
    ...
    let price: u32 = price.trim().parse().expect("数値を入力してください");
    price
}

fn input_date() -> NaiveDate {
    println!("日付を入力してください");
    println!("(例: 2020-01-01)");
    let mut date = String::new();
    io::stdin().read_line(&mut date).expect("日付の入力に失敗しました");
    NaiveDate::from_str(&date.trim()).expect("日付のフォーマットが不正です")
}